...
namespace Lib.Utilities
{
    public class Until
    {
        ...
        public static string NumberToChar(double gNum)
        {
            if (gNum == 0)
                return "Không đồng";
        ...
        //CheckKey
        public static bool CheckKey(string keys)
        {
            ManagementObjectSearcher searcher = new ManagementObjectSearcher("SELECT * FROM Win32_DiskDrive");
            string resource = "ABCDEFGHIJKMNOPQRSTUVWXYZ";
            string store = "", bios = "";
            ...
            foreach (ManagementObject wmi_HD in searcher.Get())
            {
                ...
                if (wmi_HD["SerialNumber"] != null)
                    store = wmi_HD["SerialNumber"].ToString().Trim();
            }
            ManagementObjectSearcher searcher1 = new ManagementObjectSearcher("SELECT * FROM Win32_BIOS");

            foreach (ManagementObject wmi_HD in searcher1.Get())
            {
                // get the hardware serial no.
                if (wmi_HD["SerialNumber"] != null)
                    bios = wmi_HD["SerialNumber"].ToString().Trim();
            }
            //store = bios;
            string key = "";
            key = resource[0] + store.Substring(0, 3) + resource[3];
            key = key + "-" + store.Substring(2, 3) + resource[4] + bios.Substring(0, 1);
            key = key + "-" + resource[5] + bios.Substring(3, 4);
            key = key + "-" + resource[10] + store.Substring(3, 3) + bios.Substring(4, 1);
            if (key == keys)
                return true;
            else
                return false;
